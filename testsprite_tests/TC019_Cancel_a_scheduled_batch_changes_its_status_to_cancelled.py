...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /scheduled (http://localhost:3000/scheduled) to begin verifying scheduled batch cancel flow.
        await page.goto("http://localhost:3000/scheduled")
        
        # -> Fill in the Clerk sign-in form with provided test credentials and click Continue to authenticate (then proceed to /scheduled to continue the cancel flow).
        ...
        # -> Click on the first scheduled post batch card to open its actions (click element index 1434).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/main/div/div[2]/div').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the 'Cancel' button in the batch actions menu to open the confirmation (click element index 1743).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[5]/div[3]/div/button[2]').nth(0)
        await asyncio.sleep(3); await elem.click()
