...
| `SOCIAL_TOKEN_ENCRYPTION_KEY` | Yes | Encrypts social account tokens, webhook secrets and share-link tokens at rest ([SECURITY.md](./SECURITY.md#tokens-and-secrets-at-rest)). 32 random bytes, base64: `openssl rand -base64 32`. Every environment that shares a database needs the same key, and losing it forces every account to reconnect |
