// components/core/accounts/ConnectInstagramButton.tsx
"use client";
...
declare global {
  interface Window {
    onInstagramConnectSuccess?: () => void;
    onInstagramConnectFailure?: (error?: string) => void;
  }
}
...
export default function ConnectInstagramButton({
  ...
  // Add failure handler
  const handleInstagramFailure = (error?: string) => {
    console.error("Instagram connection failed:", error);
    toast.error("Échec de la connexion au compte Instagram");
    cleanupAuthFlow();
  };

  // Setup and cleanup for window event handlers
  useEffect(() => {
    window.onInstagramConnectSuccess = handleInstagramSuccess;
    window.onInstagramConnectFailure = handleInstagramFailure;

    return () => {
      window.onInstagramConnectSuccess = undefined;
      window.onInstagramConnectFailure = undefined;
  ...
  // Open Instagram popup with unique window name
  const openInstagramPopup = async () => {
    ...
    try {
      ...
      const uniqueWindowName = `InstagramOAuth_${Date.now()}`;
