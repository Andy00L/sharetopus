...
/**
 * Writing guidance for the draft prompt. The seven dedicated platforms have
 * hand-written guidance above; registry platforms get their catalog limits
 * and title rule.
 */
function describePlatformGuidelines(platform: SchedulablePlatform): string {
  if (isPostingPlatform(platform)) return PLATFORM_GUIDELINES[platform];
  const titleNote = platformRequiresTitle(platform)
    ? " It requires a title: start with a line 'Title: <title>', then pass that title as the title argument when scheduling."
    : "";
  return `${getPlatformDisplayLabel(platform)}: stay under ${resolvePlatformTextLimit(platform)} characters and match the platform's usual style.${titleNote}`;
}
  ...
  server.registerTool(
    ...
    withMcpTool(
      ...
      async (_ctx, args: GeneratePostDraftArgs) => {
        // The platform's own ceiling wins over a larger requested length
        // (X caps at 280 while max_length defaults to 500).
        const maxDraftLength = Math.min(
          args.max_length,
          resolvePlatformTextLimit(args.platform),
        );
        const draftPrompt = [
          `Write a ${args.tone} social media post for ${getPlatformDisplayLabel(args.platform)}.`,
          `Topic: ${args.topic}`,
          `Max length: ~${maxDraftLength} characters.`,
          describePlatformGuidelines(args.platform),
